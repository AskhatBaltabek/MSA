...
class KolesaService
{
            ...
            if (isset($response)) {
                ...
                if (isset($mark[1])) {
                    $array[] = [
                        ...
                        'alias' => $mark[1], // Название в латинице
                    ];
                }
            }
        }
     ...
     * @return array
     */
    public static function GetModelParser($markId): array
    {
        $array = [];

        $data = http_build_query([
            "value" => $markId,  # this is index of a car's mark
        ...
        $response = self::_getInstance()->_request($data, 2);
        if (isset($response)) {
            foreach ($response->value as $model) {
                $alias = $model->extra->alias;
                $title = isset($model->value) ? $model->value : '';

                $pos = strpos($model->key, ',');
                if ($pos === false) {
                    $array[] = ['model_id' => (int)$model->key, 'mark_id' => $markId, 'title' => $title, 'alias' => $alias];
                }
